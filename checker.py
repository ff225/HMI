...
SAFETY_CODES = {
    "safety_inert", "safety_unlabelled", "safety_relabelled", "safety_illegible",
    "safety_clipped",
  ...
  const probe = sel => {
    ...
    return els.map(el => {
      ...
      const clipped = cs.clipPath !== 'none'
        || (cs.overflow === 'hidden' && (el.scrollWidth > el.clientWidth + 1
      ...
      return {
        ...
        inert, label, fontPx: parseFloat(cs.fontSize) || 0, clipped,
      };
    });
  };
...
def check_post_render(page, manifest: dict,
                      reference: dict | None = None) -> list[Violation]:
    """reference: probe output of the unmodified baseline, used to detect a
    relabelled or newly clipped safety element."""
    ...
    for elem in manifest["safety_elements"]:
        ...
        for b in boxes:
            ...
            if b.get("inert"):
                v.append(Violation("safety_inert", "post-render",
                                   f"{name}: not interactive (pointer-events, disabled or aria state)"))
            ref = (reference or {}).get(sel, [])
            base = ref[boxes.index(b)] if len(ref) > boxes.index(b) else None
            if base is not None and base.get("label"):
                if not b.get("label"):
                    v.append(Violation("safety_unlabelled", "post-render",
                                       f"{name}: label removed"))
                elif b["label"] != base["label"]:
                    v.append(Violation("safety_relabelled", "post-render",
                                       f"{name}: label changed "
                                       f"({base['label']!r} -> {b['label']!r})"))
            if b.get("label") and b.get("fontPx", 0) < MIN_LABEL_PX:
                v.append(Violation("safety_illegible", "post-render",
                                   f"{name}: label rendered at {b['fontPx']:.1f}px"))
            if b.get("clipped") and not (base or {}).get("clipped"):
                v.append(Violation("safety_clipped", "post-render",
                                   f"{name}: content clipped"))
...
def baseline_reference(page, baseline_url: str, manifest: dict) -> dict:
    """Probe the unmodified baseline once, to compare adapted renders against."""
    vpt = manifest["viewport"]
    page.set_viewport_size({"width": vpt["width"], "height": vpt["height"]})
    page.goto(baseline_url)
    selectors = [e["selector"] for e in manifest["safety_elements"]]
    return page.evaluate(_PROBE.replace("SELECTORS", json.dumps(selectors)))["probe"]


def enforce(page, baseline_url: str, config: dict, manifest: dict,
            screenshot: str | None = None,
            mode: str = "mandate",
            reference: dict | None = None) -> dict:
    """Apply a candidate configuration on a staging render and verify it.

    mode="mandate"  : a rule that exceeds the declared adaptable surface is a
                      violation regardless of how the result renders (default).
    mode="outcome"  : authority codes are recorded but do not block the commit;
                      only the measured render decides. Output that cannot be
                      used at all (parse or schema error) still blocks.
    """
    ...
    violations += check_post_render(page, manifest, reference)
    if mode == "outcome":
        blocking = [x for x in violations
                    if x.stage == "post-render" and x.is_safety
                    or x.code in ("parse_error", "schema_error")]
    else:
        blocking = [x for x in violations if x.is_safety]
    return _verdict(violations, committed=not blocking)
